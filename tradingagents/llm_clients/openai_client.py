...
OPENAI_COMPATIBLE_PROVIDERS: dict[str, ProviderSpec] = {
    ...
    "cohere":     ProviderSpec(base_url="https://api.cohere.com/v1"),
    "ai21":       ProviderSpec(base_url="https://api.ai21.com/studio/v1"),
    "ollama":     ProviderSpec(base_url="http://localhost:11434/v1", base_url_env="OLLAMA_BASE_URL",
                               key_optional=True, placeholder_key="ollama"),
    "lm_studio":  ProviderSpec(base_url="http://localhost:1234/v1", key_optional=True, chat_class=LocalCompatibleChatOpenAI),
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        if spec is not None:
            ...
            # Fall back to the project config store (browser-saved keys) so
            # analysis works without Coolify env vars.
            if not api_key:
                try:
                    from app.config_store import get_api_key
